...
<div class="main">
    ...
    <div class="main-content">
        <div class="container-fluid">
            ...
            <h2>Hello, <span style="color: blue"> <?php echo $_SESSION['username']?></span> Manage City Here. </h2>
            <br />
            <p><button type="button" class="btn btn-primary" data-toggle="modal" data-target="#addcity">Add new</button></p>
            <br />
                                    ...
                                    <div class="modal-body">
                                        <form action="edit_city.php?city_id=<?php echo $row['city_id']; ?>" method="post">
                                            <div class="form-group">
                                                <input type="text" name="code" id="code" class="form-control" value="<?php echo $row['city_code'] ?>">
                                            </div>
                                            <div class="form-group">
                                                <select class="form-control" name="state">
                                                    ...
                                                    while ($fetch = $state->fetch_array()) {
                                                        $selected = ($fetch['state_id'] == $row['state_fk']) ? 'selected' : '';
                                                    ?>
                                                        <option value="<?php echo $fetch['state_id'] ?>" <?php echo $selected ?>><?php echo $fetch['state_name'] ?></option>
                                                    <?php }
                                                    ?>
                                                </select>
                                            </div>
                                            <div class="form-group">
                                                <input type="text" name="name" id="name" class="form-control" value="<?php echo $row['city_name'] ?>">
                                            </div>
                                    </div>
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
